...
class ApplicantControllerTest {
  ...
  void shouldBePossibleToCreateAnApplicant() throws Exception {
    ...
    when(applicantService.save(any(Applicant.class))).thenAnswer(inv -> {
      ...
      param.setApplicantId(UUID.randomUUID());
      return param;
    });
  ...
  void get_shouldReturn404WhenNotFound() throws Exception {
    when(applicantService.getApplicant(UUID.randomUUID())).thenReturn(Optional.empty());

    mockMvc.perform(get("/applicants/{applicantId}", UUID.randomUUID()))
        .andExpect(status().isNotFound());

    verify(applicantService, times(1)).getApplicant(any(UUID.class));
  }
}